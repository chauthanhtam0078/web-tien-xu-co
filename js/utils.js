...
// BỔ SUNG: Chỉnh hàm nén ảnh để trả về thêm Tên gốc của hình ảnh (Dùng định tuyến trên thư mục Drive)
...
window.getSafeImgUrls = (url) => {
    ...
    if (match && match[1]) {
        const id = match[1];
        return {
            // ĐẢO LẠI: Dùng Thumbnail làm link chính vì cực kỳ ổn định, không bị chặn ẩn danh
...
// Hàm render thẻ img an toàn dùng chung cho toàn bộ Web & Admin
