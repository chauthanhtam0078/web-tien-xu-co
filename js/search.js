...
// Tính năng lọc tìm kiếm sản phẩm và bài viết tin tức
// ============================================================================
...
window.executeSearch = () => {
    ...
    const matchedProducts = (window.globalProducts || []).filter(p => 
        ...
        return discountB - discountA; // Xếp giảm dần
    });

    const matchedNews = (window.globalAllNews || []).filter(n => 
        (n.title && n.title.toLowerCase().includes(query)) ||
        (n.desc && window.stripHTMLForSearch(n.desc).toLowerCase().includes(query)) ||
...
function renderSearchResults(query, matchedProducts, matchedNews) {
    ...
    if (matchedProducts.length === 0) {
        ...
        matchedProducts.forEach((p, index) => {
            ...
            // Xóa HTML cho phần xem trước (Preview) ở Card
            let cleanDescPreview = window.stripHTMLForSearch(p.desc);
            ...
            <div class="w-full sm:w-[calc(50%_-_0.75rem)] lg:w-[calc(25%_-_1.125rem)] 2xl:w-[calc(20%_-_1.2rem)] bg-white border border-brand-border rounded-sm shadow-sm hover:shadow-lg transition-shadow flex flex-col relative overflow-hidden group">
                <div class="cursor-pointer flex-grow flex flex-col" onclick="window.openProductDetail('${p.id || index}')">
                    ...
                    <div class="p-5 flex-grow flex flex-col items-center text-center"><h4 class="font-serif font-bold text-lg mb-2 text-brand-dark h-14 line-clamp-2">${p.name}</h4><p class="text-sm text-gray-600 mb-4 line-clamp-2">${cleanDescPreview}</p><div class="dashed-line mt-auto w-full"></div></div>
                </div>
